...
function copyCodeBlock(option?: any) {
  ...
  const copyCodeFormats = vscode.workspace
    ...
    .get<Array<vscode.WorkspaceConfiguration>>("formats");
  ...
  if (forcePathSeparatorSlash) {
    ...
    workspaceFolderRelativePath = workspaceFolderRelativePath.replace(
      /\\/g,
      "/"
    );
  ...
  let placeHolderMap = new Map<string, ReplaceRule>([
    ...
    [
      ...
      {
        re: /\$\{workspaceFolderRelativePath\}/g,
        str: workspaceFolderRelativePath,
      },
    ],
    ["fileBasename", { re: /\$\{fileBasename\}/g, str: fileBasename }],
    ["fileExtname", { re: /\$\{fileExtname\}/g, str: fileExtname }],
    [
      "fileExtnameWithoutDot",
      {
        re: /\$\{fileExtnameWithoutDot\}/g,
        str: fileExtname.replace(/\./g, ""),
      },
    ],
    [
      "fileBasenameWithoutExtension",
      {
        re: /\$\{fileBasenameWithoutExtension\}/g,
        str: fileBasenameWithoutExtension,
      },
    ],
  ...
  selections.forEach((selection, i) => {
    ...
    for (let n = selection.start.line; n <= selection.end.line; n += 1) {
      ...
      if (forceSpaceIndent) {
        ...
        line = line.replace(
          /^(\t+)/,
          function (match: string, p1: string, offset: number, str: string) {
    ...
    if (
      ...
      let codeBlockFooter = replacePlaceHolderMap(
